         ...
         <form action="{{ route('admin.update', $games->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="row mb-3">
                <div class=col-8>
                    ...
                    <input autofocus type="text" id="name" name="name" class="form-control" required @isset($games->name) value="{{ $games->name }}" @endisset>
                </div>
                <div class=col-4>
                    <label for="developer" class="form-label">Desenvolvedora:</label>
                    <input type="text" id="developer" name="developer" class="form-control" required @isset($games->developer) value="{{ $games->developer }}" @endisset>
                </div>
            </div>
            <div class="row mb-3">
                <div>
                    <label for="description" class="form-label">Descrição do jogo:</label>
                    <textarea id="description" name="description" class="form-control" rows="4" required>@isset($games->description){{ $games->description }}@endisset</textarea>
                </div>
            </div>
            <div class="row mb-3">
                <div class=col-4>
                    <label for="genre" class="form-label">Gênero:</label>
                    <input type="text" id="genre" name="genre" class="form-control" required @isset($games->genre) value="{{ $games->genre }}" @endisset>
                </div>
                <div class=col-4>
